...
interface IHtmlPluginOptions {
  /**
   * A mapping of html shortcodes to their corresponding Unicode characters.
   * This allows for easy customization of the html representation.
   */
  htmls?: Record<string, string>;
}

/**
 * Html plugin for @m2d/core.
 * This plugin provides support for custom html transformation within Markdown content
 * during conversion to DOCX format.
 */
export const htmlPlugin: (options?: IHtmlPluginOptions) => IPlugin = options => {
  ...
  return {
    ...
    inline: async (
      ...
      if (node.type === "") {
        /**
         * TODO: Add logic to handle specific inline node types (e.g., html).
     ...
     */
    preprocess: tree => {
      // TODO: Modify or traverse the MDAST tree if needed (e.g., convert html syntax)
    },
  };
};
